...
export default async function saveCellExecution(requestMessage: APIRequestMessage): Promise<void | boolean> {
    ...
    try {
        ...
        const exitCode = runnerExitStatus ? (runnerExitStatus.type === 'exit' ? runnerExitStatus.code : -1) : 0
        ...
        const result = await graphClient.mutate({
            mutation: createCellExecutionQuery({
                ...message.output.data, exitCode, pid,
