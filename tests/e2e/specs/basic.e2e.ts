...
describe('Runme VS Code Extension', async () => {
  ...
  describe('Runme examples', async () => {
    ...
    // TODO: Check why the following tests are failing in CI
    ...
    it('basic hello world shell execution', async () => {
      ...
      expect(await cell.cellOutputExists('Hello World!', OutputType.TerminalView)).toBe(true)
    ...
    it('background task example', async () => {
      ...
      expect(await cell.isCommonStatusBarCommandsRendered()).toBe(true)
      await cell.waitForCellOutput()
      const terminalView = await cell.cellOutputExists('', OutputType.TerminalView)
      const shelloutput = await cell.cellOutputExists('', OutputType.ShellOutput)
      expect(terminalView || shelloutput).toBe(true)
    ...
    it.skip('terminal dimensions', async () => {
      const cell = await notebook.getCell('watch -n 0.1 "\necho Rows: \\$(tput lines)\necho Columns: \\$(tput cols)')
      expect(await cell.isCommonStatusBarCommandsRendered()).toBe(true)
      await cell.run()
      const regex = /Rows:\s+(\d+)\s+Columns:\s+(\d+)/
      await cell.waitForCellOutput()
      await notebook.scrollDown(3)
      expect(await cell.cellOutputExists('', OutputType.TerminalView, regex)).toBe(true)
      expect(await cell.isCommonStatusBarCommandsRendered([
        StatusBarElements.OpenTerminal
      ])).toBe(true)
    })
